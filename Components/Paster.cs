...
namespace Clippy
{
    internal static class Paster
    {
        ...
        public static EventWaitHandle GetPasteEventWaiter(int processID = 0)
        {
            if (processID == 0)
                processID = Environment.ProcessId;
            EventWaitHandle pasteEvent = new(false, EventResetMode.ManualReset, "ClippyPaste" + processID);
            return pasteEvent;
        }

        public static EventWaitHandle GetSendCharsEventWaiter(int processID = 0, bool slow = false)
        {
            if (processID == 0)
                processID = Environment.ProcessId;
            string name = "ClippySendChars";
            if (slow)
                name += "Slow";
            else
                name += "Fast";
            EventWaitHandle pasteEvent = new(false, EventResetMode.ManualReset, name + processID);
            return pasteEvent;
        }
        ...
        public static void SendChars(Main main = null, bool slow = false)
        {
            ...
            InputSimulator inputSimulator = new(); // http://inputsimulator.codeplex.com/
